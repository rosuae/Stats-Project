...
ZILE     <- 365
LAMBDA   <- 5000          # media cereri/zi (Poisson)
SCENARII <- c(0.001, 0.005, 0.02)

# Costuri pentru analiza economica
C1 <- 2     # cost per verificare
C2 <- 100   # penalizare per suspect nedetectat

# Simuleaza un an pt un scenariu p, aplica strategiile A si B
simuleaza_scenariu <- function(p_suspect) {
  total_cereri    <- rpois(ZILE, lambda = LAMBDA)           # cereri/zi ~ Poisson
  cereri_suspecte <- rbinom(ZILE, size = total_cereri, prob = p_suspect) # suspecte ~ Binomial
  ...
  # Detectie cu Hipergeometrica (extragere fara inlocuire)
...
metrici <- date_long %>%
  ...
  summarise(
    ...
    Eficienta          = sum(Detectate) / sum(Verificate) * 1000, # detectii la 1000 verificari
    .groups = "drop"
  )
print(metrici)

# Histograma cereri suspecte pe zi
...
# Histograma detectiilor A vs B (p = 0.005)
...
# Rata medie de detectie per strategie
...
# Evolutia zilnica suspecte vs detectate (p = 0.005, primele 60 zile)
...
# Analiza de costuri: C = c1 * verificari + c2 * nedetectate
costuri <- date_long %>%
  group_by(P_Scenariu, Strategie) %>%
  summarise(
...
# Salvare grafice in src/
dir.create("src", showWarnings = FALSE)

ggsave("src/Cereri_suspecte_per_zi.png", plot = g1, width = 10, height = 4, bg = "white")
ggsave("src/Cereri_suspecte_detectate.png", plot = g2, width = 8, height = 5, bg = "white")
ggsave("src/Randament.png", plot = g3, width = 8, height = 5, bg = "white")
ggsave("src/Evolutie_zilnica.png", plot = g4, width = 10, height = 4, bg = "white")

g5 <- ggplot(rezultate_q, aes(x = Procent)) +
  geom_line(aes(y = Prob_Detectie, color = "P(detectie >= 1/zi)"), linewidth = 1.2) +
  geom_point(aes(y = Prob_Detectie), size = 3) +
  geom_line(aes(y = Prop_Detectata, color = "Proportie detectata"), linewidth = 1.2, linetype = "dashed") +
  geom_point(aes(y = Prop_Detectata), size = 3) +
  ...
  labs(title = "Efectul cresterii procentului de verificare (p = 0.005)",
       x = "Procent verificare (%)", y = "Rata", color = "Metrica")
ggsave("src/Procent_verificare_modificat.png", plot = g5, width = 8, height = 5, bg = "white")

cat("\nGraficele din simulare au fost salvate cu succes in folderul src/!\n")
